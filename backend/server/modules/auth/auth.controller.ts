...
class AuthController {
    ...
    static async login(req: Request, res: Response){
        ...
        try {
            ...
            if(validationResult?.length > 0){
                ...
                console.log('credential correct', authData)
    ...
    static refreshToken(req: Request, res: Response){
        ...
        try {
            ...
            const {id, email, department_id, firstName, lastName, phone, iat, exp } = user as jwt.JwtPayload
            if(iat && exp && iat  < exp){
    ...
    static async updateUserData(req: Request, res: Response){
        ...
        const result = await authDao.updateUserData(req.body)
        if(result){
